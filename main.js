...
function productCardHTML(p) {
  ...
  var detailUrl = 'product.html?id=' + encodeURIComponent(p.id);

  return '<article class="product-card">' +
    '<a class="product-img product-image-link" href="' + detailUrl + '" aria-label="View ' + sanitize(p.name) + '">' +
    ...
    '<div class="product-info">' +
      '<div class="part-no">' + sanitize(p.partNo) + '</div>' +
      '<h3><a class="product-title-link" href="' + detailUrl + '">' + sanitize(p.name) + '</a></h3>' +
      '<p class="product-desc">' + sanitize((p.desc || '').slice(0, 80)) + (p.desc && p.desc.length > 80 ? '…' : '') + '</p>' +
      '<div class="product-specs">' +
        '<div class="spec-row"><span>Part No</span><strong>' + sanitize(p.partNo || '—') + '</strong></div>' +
        '<div class="spec-row"><span>Brand</span><strong>' + sanitize(p.brand || '—') + '</strong></div>' +
        '<div class="spec-row"><span>Stock</span><strong>Ready Stock</strong></div>' +
        '<div class="spec-row"><span>Availability</span><strong>7–10 Business Days</strong></div>' +
        '<div class="spec-row"><span>Shipping</span><strong>Worldwide</strong></div>' +
      '</div>' +
      '<div class="product-actions">' +
        '<a class="pact-btn pact-details" href="' + detailUrl + '">View details</a>' +
...
/* Compact catalogue card: product information and enquiry actions live on product.html. */
function productCardHTML(p) {
  var detailUrl = 'product.html?id=' + encodeURIComponent(p.id);
  var image = p.img
    ? '<img src="' + p.img + '" alt="' + sanitize(p.name) + '" class="real-photo" onerror="this.style.display=\'none\'">'
    : '<div class="svg-wrap">' + getProductSVG(p.imgType || 'kit') + '</div><div class="part-no-display">' + sanitize(p.partNo) + '</div>';

  return '<article class="product-card catalogue-card">' +
    '<a class="product-img product-image-link" href="' + detailUrl + '" aria-label="View ' + sanitize(p.name) + '">' +
      '<span class="product-brand-badge">' + sanitize(p.brand) + '</span>' + image +
    '</a>' +
    '<div class="product-info">' +
      '<div class="part-no">' + sanitize(p.partNo) + '</div>' +
      '<h3><a class="product-title-link" href="' + detailUrl + '">' + sanitize(p.name) + '</a></h3>' +
      '<div class="catalogue-card-footer">' +
        '<a href="' + detailUrl + '" class="catalogue-details-link">View product</a>' +
        '<button class="catalogue-cart" title="Add to cart" aria-label="Add ' + sanitize(p.name) + ' to cart" onclick="handleAddCart(\'' + String(p.id).replace(/'/g, "\\'") + '\', this)">' +
          '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 002 1.61h9.72a2 2 0 001.97-1.67L23 6H6"/></svg>' +
        '</button>' +
      '</div>' +
    '</div>' +
  '</article>';
